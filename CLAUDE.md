...
- Unresolved: (1) labour closure F-4/F-6/F-11 is circular (`AVGW·1e-9 = 0`), so the nominal net-wage level is only pinned by an epsilon guard — harmless while `W = 1` (F-12) anchors costs, but a proper wage curve/market clearing would be a modelling change; (2) `RecursiveDynamic.jl::update_period_data!` mixes units — `KSupply` is a rental payment (CAP/2 per vintage at the benchmark), `FDInv` a commodity flow — so `K0` jumps ≈2× per period and PATH fails from period 3; (3) trade margins are zero at the benchmark (`zeta_t = 0`, the SAM's margin account is folded into exports/imports).
