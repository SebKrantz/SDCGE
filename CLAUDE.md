...
- Unresolved: (1) **labour closure** — `W = 1` (F-12) is fixed, F-10 makes `UE` absorb supply−demand, F-7 `(TW−WMIN)·UE = 0 ⟂ UE` is degenerate with `TW ≡ WMIN ≡ 1`, and F-4/F-6/F-11 are circular (`AVGW·1e-9 = 0`); labour supply is not binding, so `g_labor` only creates unemployment and the default 10-period run fails 7/10 periods. Fix in `Factors.jl`: market-clearing wage (`ΣLV = LS·(1−UE) ⟂ W`, `UE` exogenous) or a wage curve replacing F-6/F-9. Warm-starting periods from the previous solution was tried and made things worse (comment in `Initialization.jl`). (2) `AT` enters P-1/P-2 as `AT·…` but P-3 as `…/AT` (CES demand should carry `AT^(σ−1)`); only matters away from `AT = 1`. (3) trade margins are zero at the benchmark (`zeta_t = 0`).
